...
export function activate(context: ExtensionContext) {
	...
	const serProvider = new SerialProvider();
    window.registerTreeDataProvider('SerialDeviceProviderService', serProvider);
	
	commands.registerCommand('serialdevices.DevicesSerial',serProvider.onFileClicked);
    commands.registerCommand('serialdevices.getsome', serProvider.getdevices);
	commands.registerCommand("serialdevices.refreshEntry", () =>serProvider.dorefresh());
	 commands.registerCommand('serialdevices.editEntry', (node: SerialD) =>  window.showInformationMessage(`Successfully called edit entry on ${node.label}.`));
	commands.registerCommand('serialdevices.renameEntry', (item:SerialD)=>serProvider.tryrename(item));
	//see output
